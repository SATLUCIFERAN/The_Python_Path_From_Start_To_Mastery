...
def vee(n, w, h):
    ...
    mid = (n-1)/2
    return [(cx + (i-mid)*(span/n), cy + abs(i-mid)*(span/(n*1.6))) for i in range(n)]
...
def xwing(n, w, h):
    ...
    def seg(a,b,k):
        ax,ay=a; bx,by=b
        return [(ax+(i+1)/(k+1)*(bx-ax), ay+(i+1)/(k+1)*(by-ay)) for i in range(k)]
...
def draw(ships):
    ...
    print("X-Wing Aerobatic Display — Line → V → Diamond → X → Mirror → Circle (Ctrl+C to exit)")
