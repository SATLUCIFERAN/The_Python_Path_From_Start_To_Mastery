...
import yaml  # New library to read YAML files
import time
...
# IMPORTANT: don't mutate `defaults`
config = defaults.copy()
# .env overrides defaults
config.update({k.lower(): v for k, v in env_config.items()})
# YAML (mission) overrides .env and defaults
...
def main(
    since: int = typer.Option(
        int(config.get('since_days', 7)),  # Now defaults to the merged config
        "--since",
        "-s",
        help="The number of days of data to process.",
    ),
    out: str = typer.Option(
        config.get('out_path', 'report.csv'),  # Now defaults to the merged config
        "--out",
        "-o",
    ...
    # --- Step 3: Droid delivers the report safely ---
    try:
        console.print(f"[green]Saving report to: [bold]{out}[/bold]...")
        out_path = Path(out)
        out_path.parent.mkdir(parents=True, exist_ok=True)

        with open(out_path, "w") as f:
            f.write("Mission Report\n")
            f.write(f"Data processed from {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}\n")
