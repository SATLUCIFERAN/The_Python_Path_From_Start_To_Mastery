...
def lock_file(path: Path):
    """
    This is a context manager that safely handles a lockfile.
    It attempts to acquire the lock, and if successful, yields to the user.
    On exit, it ensures the lock is released.
    """
    console.print(f"[cyan]Attempting to acquire lock...[/cyan]")
    lock_path = Path(path)
    if lock_path.exists():
        console.print("[red]Lock file exists. Another instance may be running. Aborting.[/red]")
        yield False
    else:
        try:
            lock_path.touch()
            console.print("[green]Lock acquired. Proceeding with mission.[/green]")
            yield True
        finally:
            if lock_path.exists():
                lock_path.unlink()
                console.print("[green]Lock released. Mission complete.[/green]")
...
def main(
    ...
    log: str = typer.Option(
        config.get('log_path', 'run_log.jsonl'),
        "--log",
        "-l",
        help="The path to save the run log."
    ),
    lock: str = typer.Option(
        config.get('lock_path', '.lock'),
        "--lock",
        help="The path to the lockfile."
    ),
):
    ...
    console.print(f"  [bold]Hardwired Default:[/bold] Since={defaults['since_days']}, Out={defaults['out_path']}, Keep={defaults['keep']}, Log={defaults['log_path']}, Lock={defaults['lock_path']}")
    console.print(f"  [bold]Env Orders:[/bold] {env_config}")
    console.print(f"  [bold]YAML Briefing:[/bold] {yaml_config}")
    console.print(f"  [bold]Final Orders:[/bold] Since={since}, Out={out}, Dry Run={dry_run}, Keep={keep}, Log={log}, Lock={lock}\n")
    ...
    # Place a lock to ensure only one instance can run at a time
    with lock_file(Path(lock)) as locked:
        if not locked:
            log_run(
                status="FAILURE",
                message="Mission aborted: Lock file already exists.",
                metadata=log_metadata,
                log_path=Path(log)
            )
            sys.exit(1)

        # The rest of the mission logic now lives inside the lock
        if dry_run:
